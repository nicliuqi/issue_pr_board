...
func (c *IssuesController) Get() {
	...
	if validateErr := validate.Struct(qp); validateErr != nil {
		logs.Error("[IssuesController] Fail to validate params, err:", validateErr)
	...
	if _, err := o.Raw(sql, sqlParams).QueryRows(&issue); err != nil {
		logs.Error("[IssuesController] Fail to query issues, err:", err)
	...
	for _, i := range issue {
		...
		if err != nil {
			logs.Error("[IssuesController] Fail to decode raw description, err:", err)
			continue
		}
...
func (c *IssueNewController) Post() {
	logs.Info("[IssueNewController] Receive a request of creating an issue")
	headers := c.Ctx.Request.Header
	contentType, ok := headers["Content-Type"]
	if !ok {
		logs.Error("[IssueNewController] The request has no header 'Content-Type'")
		c.ApiJsonReturn("Bad request", http.StatusBadRequest, nil)
	}
	if !strings.HasPrefix(contentType[0], "application/json") {
		logs.Error("[IssueNewController] The request header 'Content-Type' does not match 'application/json'")
		c.ApiJsonReturn("Bad request", http.StatusBadRequest, nil)
	}
	reqBody, err := io.ReadAll(c.Ctx.Request.Body)
	if err = c.Ctx.Request.Body.Close(); err != nil {
		logs.Error("[IssueNewController] Fail to close request body of getting request to create an issue, err:",
			err)
	...
	if err = json.Unmarshal(reqBody, &params); err != nil {
		logs.Error("[IssueNewController] Fail to unmarshal request, err:", err)
		c.ApiJsonReturn("Invalid params", http.StatusBadRequest, nil)
	}
	validate := validator.New()
	if validateErr := validate.Struct(params); validateErr != nil {
		logs.Error("[IssueNewController] Fail to validate params, err:", validateErr)
		c.ApiJsonReturn("Invalid params", http.StatusBadRequest, nil)
	}
	...
	if privacy != true {
		logs.Error("[IssueNewController] The user does not agree the privacy policy before creating an issue, user"+
			"email:", annoyAddr)
		c.ApiJsonReturn("Invalid params", http.StatusBadRequest, nil)
	}
	logs.Info(fmt.Sprintf("User whose email address is %v had agreed the privacy to create an issue", annoyAddr))
	if !models.CheckCode(addr, code) {
		logs.Error(fmt.Sprintf("[IssueNewController] Fail to check code, addr: %v, code: %v", annoyAddr, code))
	...
	if err != nil {
		logs.Error("[IssueNewController] Fail to marshal request body, err:", err)
	...
	requestUrl := fmt.Sprintf("%v/enterprises/%v/issues", config.AppConfig.GiteeV8ApiPrefix, enterpriseId)
	req, err := http.NewRequest("POST", requestUrl, payload)
	if err != nil {
		logs.Error("[IssueNewController] Fail to wrap the request object, err:", err)
		c.ApiJsonReturn("Fail to create an issue", http.StatusBadRequest, nil)
	}
	req.Header.Add("Content-Type", "application/json")
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		logs.Error("[IssueNewController] Fail to send post request to create an issue, err:", err)
		c.ApiJsonReturn("Fail to create an issue", http.StatusBadRequest, nil)
	}
	if resp.StatusCode != http.StatusCreated {
		logs.Error("[IssueNewController] Get unexpected status when creating an issue, detail:", resp.Status)
		c.ApiJsonReturn("Fail to create an issue", http.StatusBadRequest, nil)
	}
	content, _ := io.ReadAll(resp.Body)
	if err = resp.Body.Close(); err != nil {
		logs.Error("[IssueNewController] Fail to close response body of creating an issue, err:", err)
		c.ApiJsonReturn("Fail to close response body of creating issues", http.StatusBadRequest, nil)
	}

	var res NewIssueResponse
	if err = json.Unmarshal(content, &res); err != nil {
		logs.Error("[IssueNewController] Fail to unmarshal response, err:", err)
	...
	if !models.SearchIssueRecord(number) {
		...
		if _, err = o.Insert(&issue); err != nil {
			logs.Error("[IssueNewController] Fail to create issue with reporter, err:", err)
			c.ApiJsonReturn("Fail to create an issue", http.StatusBadRequest, nil)
		}
	} else {
		if _, err = o.QueryTable("issue").Filter("number", number).Update(orm.Params{
			...
			logs.Error(fmt.Sprintf("[IssueNewController] Fail to update issue, issue number: %v,"+
				"err: %v", number, err))
			c.ApiJsonReturn("Fail to update issue", http.StatusBadRequest, nil)
		}
	}

	ip, _ := headers["X-Forwarded-For"]
	logs.Info(fmt.Sprintf("[IssueNewController] An Issue had been created by user whose email address"+
		"is %v, request ip: %v, issue number: %v, result: success", annoyAddr, ip, number))

	cleanCode(addr, code)

	go NewIssueNotify(projectId, number, issueUrl)

	c.ApiJsonReturn("[IssueNewController] Success to create an issue", http.StatusOK, result)
}
...
func (c *AuthorsController) Get() {
	...
	validate := validator.New()
	if validateErr := validate.Struct(params); validateErr != nil {
		logs.Error("[AuthorsController] Fail to validate params, err:", validateErr)
		c.ApiJsonReturn("Invalid params", http.StatusBadRequest, nil)
	}
	offset := perPage * (page - 1)
	o := orm.NewOrm()
	...
	if err != nil {
		logs.Error("[AuthorsController] Fail to query issue authors, err:", err)
		c.ApiJsonReturn("Query error", http.StatusBadRequest, nil)
	}
	separateSql := sql + fmt.Sprintf(" limit ? offset ?")
	if _, err = o.Raw(separateSql, perPage, offset).QueryRows(&issue2); err != nil {
		logs.Error("[AuthorsController] Fail to query issue authors with limits, err:", err)
		c.ApiJsonReturn("Query error", http.StatusBadRequest, nil)
	}
	res := make([]string, 0)
...
func (c *AssigneesController) Get() {
	...
	validate := validator.New()
	if validateErr := validate.Struct(params); validateErr != nil {
		logs.Error("[AssigneesController] Fail to validate params, err:", validateErr)
		c.ApiJsonReturn("Invalid params", http.StatusBadRequest, nil)
	}
	offset := perPage * (page - 1)
	o := orm.NewOrm()
	...
	count, err := o.Raw(sql).QueryRows(&issue)
	if err != nil {
		logs.Error("[AssigneesController] Fail to query issue assignees, err:", err)
		c.ApiJsonReturn("Query error", http.StatusBadRequest, nil)
	}
	separateSql := sql + fmt.Sprintf(" limit ? offset ?")
	if _, err = o.Raw(separateSql, perPage, offset).QueryRows(&issue2); err != nil {
		logs.Error("[AssigneesController] Fail to query issue assignees with limits, err:", err)
		c.ApiJsonReturn("Query error", http.StatusBadRequest, nil)
	}
	res := make([]string, 0)
	for _, i := range issue2 {
		if i.Assignee != "" {
			res = append(res, url.QueryEscape(i.Assignee))
		}
	}
	if keyWord == "" {
		c.ApiDataReturn(count, page, perPage, res)
		...
		for _, j := range issue {
			if strings.Contains(strings.ToLower(j.Assignee), strings.ToLower(keyWord)) {
				newRes = append(newRes, url.QueryEscape(j.Assignee))
			}
		}
		count = int64(len(newRes))
		finalRes := make([]string, 0)
...
func (c *BranchesController) Get() {
	...
	validate := validator.New()
	if validateErr := validate.Struct(params); validateErr != nil {
		logs.Error("[BranchesController] Fail to validate params, err:", validateErr)
		c.ApiJsonReturn("Invalid params", http.StatusBadRequest, nil)
	}
	offset := perPage * (page - 1)
	o := orm.NewOrm()
	...
	count, err := o.Raw(sql).QueryRows(&issue)
	if err != nil {
		logs.Error("[BranchesController] Fail to query issue branches, err:", err)
		c.ApiJsonReturn("Query error", http.StatusBadRequest, nil)
	}
	separateSql := sql + fmt.Sprintf(" limit ? offset ?")
	if _, err = o.Raw(separateSql, perPage, offset).QueryRows(&issue2); err != nil {
		logs.Error("[BranchesController] Fail to query issue branches with limits, err:", err)
		c.ApiJsonReturn("Query error", http.StatusBadRequest, nil)
	}
	res := make([]string, 0)
	for _, i := range issue2 {
...
func (c *MilestonesController) Get() {
	...
	validate := validator.New()
	if validateErr := validate.Struct(params); validateErr != nil {
		logs.Error("[MilestonesController] Fail to validate params, err:", validateErr)
		c.ApiJsonReturn("Invalid params", http.StatusBadRequest, nil)
	}
	o := orm.NewOrm()
	var sql string
	...
	if _, err := o.Raw(sql).QueryRows(&issue); err != nil {
		logs.Error("[MilestonesController] Fail to query issue milestones, err:", err)
		c.ApiJsonReturn("Query error", http.StatusBadRequest, nil)
	}
	res := make([]string, 0)
	for _, i := range issue {
...
func (c *LabelsController) Get() {
	...
	validate := validator.New()
	if validateErr := validate.Struct(params); validateErr != nil {
		logs.Error("[LabelsController] Fail to validate params, err:", validateErr)
		c.ApiJsonReturn("Invalid params", http.StatusBadRequest, nil)
	}
	o := orm.NewOrm()
	var sql string
	...
	if _, err := o.Raw(sql).QueryRows(&issue); err != nil {
		logs.Error("[LabelsController] Fail to query issue labels, err:", err)
		c.ApiJsonReturn("Query error", http.StatusBadRequest, nil)
	}
	res := make([]string, 0)
	for _, i := range issue {
...
func (c *TypesController) Get() {
	...
	validate := validator.New()
	if validateErr := validate.Struct(qp); validateErr != nil {
		logs.Error("[TypesController] Fail to validate params, err:", validateErr)
		c.ApiJsonReturn("Invalid params", http.StatusBadRequest, nil)
	}
	o := orm.NewOrm()
	sql, sqlParams := formQueryIssueTypesSql(qp)
	if _, err := o.Raw(sql, sqlParams).QueryRows(&issueTypes); err != nil {
		logs.Error("[TypesController] Fail to query issue types, err:", err)
		c.ApiJsonReturn("Query error", http.StatusBadRequest, nil)
	}
...
func (c *UploadImageController) Post() {
	logs.Info("[UploadImageController] Ready to upload a image")
	headers := c.Ctx.Request.Header
	contentType, ok := headers["Content-Type"]
	if !ok {
		logs.Error("[UploadImageController] The request has no header 'Content-Type'")
		c.ApiJsonReturn("Bad request", http.StatusBadRequest, nil)
	}
	if !strings.HasPrefix(contentType[0], "multipart/form-data") {
		logs.Error("[UploadImageController] The request header 'Content-Type' does not match 'multipart/form-data'")
		c.ApiJsonReturn("Bad request", http.StatusBadRequest, nil)
	}
	file, h, err := c.GetFile("file")
	if err != nil {
		logs.Error("[UploadImageController] Fail to read the uploaded file, err:", err)
		c.ApiJsonReturn("Fail to read content of the upload file", http.StatusBadRequest, nil)
	}
	if file == nil {
		logs.Error("[UploadImageController] The file must not be empty")
		c.ApiJsonReturn("Bad request", http.StatusBadRequest, nil)
	}
	defer func(file multipart.File) {
		if err = file.Close(); err != nil {
			logs.Error("[UploadImageController] Fail to close response of reading the uploaded file, err:", err)
	...
	if !matchUploadFileSuffix(path.Ext(h.Filename)) {
		logs.Error("[UploadImageController] The uploaded file does not match the expected type")
		c.ApiJsonReturn("Invalid type of upload file", http.StatusBadRequest, nil)
	}
	if !matchUploadFileSize(h.Size) {
		logs.Error("[UploadImageController] The uploaded file does not match the expected size")
	...
	if err != nil {
		logs.Error("[UploadImageController] Cannot read buf of the uploaded file, err:", err)
	...
	if _, err = encoder.Write(content); err != nil {
		logs.Error("[UploadImageController] Cannot encode the uploaded file, err:", err)
	...
	if err != nil {
		logs.Error("[UploadImageController] Fail to marshal request body, err:", err)
		c.ApiJsonReturn("Fail to marshal request body", http.StatusBadRequest, nil)
	}
	payload := strings.NewReader(string(requestBodyByte))
	requestUrl := fmt.Sprintf("%v/enterprises/%v/attach_files/upload_with_base_64?access_token=%s",
		config.AppConfig.GiteeV8ApiPrefix, config.AppConfig.EnterpriseId, config.AppConfig.V8Token)
	req, err := http.NewRequest("POST", requestUrl, payload)
	if err != nil {
		logs.Error("[UploadImageController] Fail to wrap the request object, err:", err)
		c.ApiJsonReturn("Fail to request", http.StatusBadRequest, nil)
	}
	req.Header.Add("Content-Type", "application/json")
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		logs.Error("[UploadImageController] Fail to send post request, err:", err)
		c.ApiJsonReturn("Fail to upload image file", http.StatusBadRequest, nil)
	}
	defer func(Body io.ReadCloser) {
		if err = Body.Close(); err != nil {
			logs.Error("[UploadImageController] Fail to close response body of uploading file, err:", err)
	...
	if err = json.Unmarshal(result, &uploadImageResponse); err != nil {
		logs.Error("[UploadImageController] Fail to unmarshal response, err:", err)
...
func NewIssueNotify(enterpriseNumber int, number, link string) {
	...
	if err := config.LoadFromYaml("conf/new_issue_notify.yaml", notifyConf); err != nil {
		logs.Error("[NewIssueNotify] Fail to load notify yaml, err:", err)
		return
	}
		...
		if name != sigName {
			...
			for _, receiver := range receivers {
				...
				if err := utils.SendNewIssueNotifyEmail(ep); err != nil {
					logs.Error("[NewIssueNotify] Fail to send new issue notify, err:", err)
				}
			}
			break
		}
